...
public class URLTest {
    ...
    public void test2ValurOf(){
        URL url = URL.valueOf("direct://127.0.0.1:8002/com.weibo.api.motan.RegistryService?group=default_rpc");
        Assert.assertNotNull(url);
        Assert.assertEquals("127.0.0.1",url.getHost());
        Assert.assertEquals(8002,url.getPort());
        Assert.assertEquals("direct",url.getProtocol());
        Assert.assertEquals("com.weibo.api.motan.RegistryService",url.getPath());
